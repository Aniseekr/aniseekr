...
describe('pilgrimage design flow helpers', () => {
  ...
  it('loads map bounds for any valid box (queries are local now, no API to protect)', () => {
    ...
    expect(shouldLoadPilgrimageMapBounds({ south: 24, west: 122.9, north: 45.6, east: 146 })).toBe(
      true
    );
    ...
    expect(shouldLoadPilgrimageMapBounds({ south: 45, west: 139, north: 35, east: 140 })).toBe(
      false
    );
    expect(shouldLoadPilgrimageMapBounds({ south: NaN, west: 139, north: 35, east: 140 })).toBe(
      false
    );
  });
});
