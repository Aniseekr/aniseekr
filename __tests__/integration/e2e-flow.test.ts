...
suite('AnimeRepository E2E', () => {
  it(
    'E2E-001 search "Bebop" then fetchAnimeDetail returns matching item',
    async () => {
  ...
  it(
    'E2E-002 switching browseSource produces a different cache key for seasonal',
    async () => {
      queryClient.invalidateAll();
      await CacheService.init();
      await CacheService.clear();
  ...
  it(
    'E2E-003 detail with bangumi platformData yields pilgrimage data via PilgrimageRepository',
    async () => {
      queryClient.invalidateAll();
      await CacheService.init();
      await CacheService.clear();
  ...
  it(
    'E2E-004 fetchAnimeStaff with sourcePlatform=bangumi falls back to Jikan',
    async () => {
      queryClient.invalidateAll();
      await CacheService.init();
      await CacheService.clear();
      ...
      const { idMappingService } = await import(
        '../../libs/services/sync/id-mapping-service'
      );
      idMappingService.setManualMapping(
        'bangumi',
        HYOUKA_BANGUMI_ID,
        'myanimelist',
        HYOUKA_MAL_ID
